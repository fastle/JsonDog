...
bool JsonTreeModel::loadJson(const QString &now)
{
    const QByteArray raw_data=now.toUtf8();
...
QString JsonTreeModel::dumpJson()
{
    ...
    QByteArray simpbyte_array = json_doc.toJson(QJsonDocument::Indented);

    qDebug()<<"dump json file";
    return QString(simpbyte_array);
}
